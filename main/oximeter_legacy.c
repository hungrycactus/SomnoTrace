...
static const char *sync_err_str(legacy_sync_err_t r)
{
    switch (r) {
    case LEGACY_SYNC_OK:         return "ok";
    case LEGACY_SYNC_ERR_CONNECT:return "connect-failed";
    case LEGACY_SYNC_ERR_INFO:   return "info-failed";
    case LEGACY_SYNC_ERR_IDENTITY: return "serial-mismatch";
    case LEGACY_SYNC_ERR_TRANSFER: return "transfer-failed";
    }
    return "unknown";
}
                                      ...
                                      int *out_files_pulled)
{
    ...
    if (!s_op_sem || !s_conn_sem || !s_resp_sem) {
        ...
        return LEGACY_SYNC_ERR_CONNECT;
    }
    ...
    if (connect_and_discover(addr) != ESP_OK) {
        do_disconnect();
        ESP_LOGI(TAG, "session end: %s", sync_err_str(LEGACY_SYNC_ERR_CONNECT));
        return LEGACY_SYNC_ERR_CONNECT;
    }
    ...
    if (legacy_get_info(&info) != ESP_OK || info.sn[0] == '\0') {
        ...
        do_disconnect();
        ESP_LOGI(TAG, "session end: %s", sync_err_str(LEGACY_SYNC_ERR_INFO));
        return LEGACY_SYNC_ERR_INFO;
    }
    ...
    if (expect_serial && strcmp(info.sn, expect_serial) != 0) {
        ...
        do_disconnect();
        ESP_LOGI(TAG, "session end: %s", sync_err_str(LEGACY_SYNC_ERR_IDENTITY));
        return LEGACY_SYNC_ERR_IDENTITY;
    }

    if (out_serial && serial_sz > 0)
        strlcpy(out_serial, info.sn, serial_sz);

    legacy_sync_err_t result = LEGACY_SYNC_OK;
    int pulled = 0;
    if (download_files) {
        for (int i = 0; i < info.nfiles; i++) {
            ...
            if (pull_file(info.sn, name) != ESP_OK) {
                ...
                result = LEGACY_SYNC_ERR_TRANSFER;
